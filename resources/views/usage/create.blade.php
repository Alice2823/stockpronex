<x-app-layout>
    ...
    <script>
        ...
        document.getElementById('quantity').addEventListener('input', debouncedRefreshBusinessFields);
        ...
        function refreshBusinessFields() {
            ...
            if (qty > 10000) {
                alert('{{ __("Quantity is too large for individual entry. Please enter a value under 10000.") }}');
            ...
            container.innerHTML = `
                <div class="flex flex-col items-center justify-center py-10 bg-gray-50 dark:bg-gray-800/40 rounded-2xl border border-gray-100 dark:border-gray-700">
                    ...
                    <p class="text-sm font-bold text-gray-500 dark:text-gray-400">{{ __('Generating forms... Please wait.') }}</p>
                </div>
            `;
            ...
            function processChunk() {
                ...
                const end = Math.min(current + CHUNK_SIZE, qty);
                ...
                if (current < qty) {
                    renderTimeout = setTimeout(processChunk, 15);
                } else {
                    isRendering = false;
                }
            }
        ...
        const debouncedRefreshBusinessFields = debounce(() => {
            isRendering = false;
            refreshBusinessFields();
        }, 500);
