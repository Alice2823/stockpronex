            ...
            @if($stocks->count() > 0)
                <div class="space-y-4">
                    @foreach($stocks as $stock)
                        ...
                        <div class="bg-white dark:bg-gray-900 overflow-hidden shadow sm:rounded-xl border border-gray-100 dark:border-gray-800 transition-all hover:shadow-lg flex flex-col sm:flex-row justify-between items-center p-5 relative overflow-hidden group">
                            
                            <!-- Small Accent bar on left -->
                            <div class="absolute left-0 top-0 bottom-0 w-1 {{ $stock->calculated_profit > 0 ? 'bg-green-500' : ($stock->calculated_profit < 0 ? 'bg-red-500' : 'bg-gray-300 dark:bg-gray-700') }}"></div>
